...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    buildcontrol: {
      options: {
        ...
        message:
          'Built using Angular Fullstack v<%= pkg.version %> from commit %sourceCommit%',
      },
      release: {
        options: {
          remote: 'origin',
  ...
  grunt.registerTask('commitNgFullstackDeps', function () {
    ...
    if (Array.isArray(ops.files) && ops.files.length > 0) {
      gitCmd(
        ['commit', '-m', version].concat(ops.files),
        {
  ...
  grunt.registerTask('demo', ['clean:demo', 'generateDemo']);
